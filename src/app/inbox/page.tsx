...
export default function InboxPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold px-4 py-4 border-b">📥 Inbox</h1>

      {isLoading && <p className="p-4 text-gray-500">Loading...</p>}
      {error && <p className="p-4 text-red-500">Error: {String(error)}</p>}

      {allMessages.length === 0 && !isLoading && (
        <p className="p-4 text-gray-500">No messages in inbox.</p>
      )}

      <div>
        {allMessages.map((email) => (
          <EmailRow key={email.id} email={email} context="inbox" href={`/inbox/${email.id}`} />
        ))}
      </div>
