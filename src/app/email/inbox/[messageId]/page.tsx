...
export default function InboxMessagePage({
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen bg-[#0e1116] text-white relative">
      {/* Background list (hidden or slightly dimmed when overlay is active) */}
      <div className={`p-8 pb-4 max-w-5xl transition-opacity duration-300 ${messageId ? "opacity-30 pointer-events-none select-none blur-sm" : ""}`}>
        ...
        <div className="flex space-x-6 border-b border-gray-800 pb-3 mb-6">
          <Link href="/email/inbox" className="flex items-center space-x-2 text-[#5c4dff] font-medium px-4 py-2 bg-[#5c4dff]/10 rounded-lg">
            <span>Inbox</span>
          </Link>
          <Link href="/email/sent" className="flex items-center space-x-2 text-[#8b949e] font-medium px-4 py-2 rounded-lg">
            <span>Sent</span>
          </Link>
          <Link href="/email/drafts" className="flex items-center space-x-2 text-[#8b949e] font-medium px-4 py-2 rounded-lg">
            <span>Drafts</span>
          </Link>
        </div>
